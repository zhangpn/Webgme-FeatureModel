...
define([], function () {
    ...
    UseCaseDiagramImporter.prototype.buildDiagram = function (fmData, modelNotation) {
        var self = this,
        ...
        for (i = 0; i < fmData.packagedElement.length; i += 1) {
            ...
            nodeType = fmData.packagedElement[i][PREFIX + 'type'].replace('uml:', '');


            if (nodeType === 'Association') {

                links.push({
                    src: fmData.packagedElement[i]['ownedEnd'][1]['@type'].replace('src_', ''),
                    dst: fmData.packagedElement[i]['ownedEnd'][0]['@type'].replace('dst_', ''),
                    type: 'Association'
                });
                ...
                self.core.setAttribute(node, 'name', fmData.packagedElement[i]['@name']);
                ...
                if (fmData.packagedElement[i].extend) {
                    if (fmData.packagedElement[i].extend.length) {

                        for (j = 0; j < fmData.packagedElement[i].extend.length; j += 1) {
                            links.push({
                                src: nodeId,
                                dst: fmData.packagedElement[i].extend[j]['@extendedCase'],
                                type: 'Extend'
                            });
                        }
                    } else {
                        links.push({
                            src: nodeId,
                            dst: fmData.packagedElement[i].extend['@extendedCase'],
                            type: 'Extend'
                        });
                    }
                }
                // Gather the outgoing include links from the current use case and store the info.
                if (fmData.packagedElement[i].include) {
                    if (fmData.packagedElement[i].include.length) {

                        for (j = 0; j < fmData.packagedElement[i].include.length; j += 1) {
                            links.push({
                                src: nodeId,
                                dst: fmData.packagedElement[i].include[j]['@addition'],
                                type: 'Include'
                            });
                        }
                    } else {
                        links.push({
                            src: nodeId,
                            dst: fmData.packagedElement[i].include['@addition'],
                            type: 'Include'
                        });
                    }
                }
